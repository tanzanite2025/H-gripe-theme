...
- 若已经生成 Stripe/PayPal 渠道提交快照，物流纠正不能覆盖该快照；后续要提交修订材料时，必须创建新的订单证据包修订版和新的渠道快照。
...
实时查看允许看到最新同步到的物流事实。首次拒付提交前，支付适配器先把统一组装结果、渠道字段、来源 ID、附件 SHA-256 和物流上下文写入 `order_evidence_submission_snapshots`，计算 SHA-256 后锁定，再调用 Stripe/PayPal。后续失败重试必须先读取同一渠道、同一 dispute 的锁定快照，不能重新读取订单、证据、物流、客服沟通，也不能重新生成或上传 PayPal 商业发票；本次请求传入的新备注、文件 ID 或其他证据字段不覆盖锁定版本。提交审计和返回结果记录快照 ID、版本和 SHA-256。
...
- 保持现有 `Orders.vue`、订单 handler 和支付拒付服务的现有职责。
- 为订单金额、订单行和产品履约要求读取补充边界测试。
- 把现有 `SignatureRequired` 使用的 750 USD 判定抽象成唯一的高价订单策略；证据包和发货签名都读取同一个结果，不各自复制阈值计算。
- 识别并拆分订单创建服务中与快照构建无关的后置动作。
- 不把规则、附件上传和订单证据编排继续堆进旧的订单页面；后续能力继续沿独立入口扩展。

### 第 1 步：订单快照契约

- 保存订单级和订单行级产品、规格、价格、重量和产品履约要求快照。
- 固化订单最终总额、USD 等值金额和金额规则版本。
- 为快照增加 schema version 和 SHA-256。
- 不改 `QUICKBUY` 领域；所有订单入口统一进入同一套订单快照契约。

### 第 2 步：独立订单证据域

- 新增独立 domain、repository、service、handler 和迁移文件。
- 由后端根据订单事实计算基础、高价值和轮组条件证据。
- 先支持结构化记录和附件引用，后台基础履约证据页面已接入，复杂拒付适配仍保持独立。
...
- 已新建独立 API 客户端、类型、订单证据页面/弹窗，以及产品履约要求页面/弹窗。
- 订单号选择必须读取真实订单并检查权限。
- 订单“发货”按钮打开的弹窗统一录入身份、包装和必要的张力表证据；上传必须经过证据附件服务，不能由前端直接拼接存储 URL。
- 独立履约证据 TAB 继续用于跨订单检索、查看和补录，包括发货后的人工 POD。
