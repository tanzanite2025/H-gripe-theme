...
本项目仍处在 WordPress/PHP 到 Go 的迁移过程中。PHP 目录保留的意义是兼容旧线上能力和查询旧业务逻辑，不是新增功能入口。

硬性规则：

1. 不在 `functions.php` 或 `wp-plugin/**` 添加、修改、回调核心业务逻辑。
2. 所有新业务逻辑进入 `go-backend`。
3. 当前管理后台主线是 `go-backend/web/admin`，不是 `go-backend/admin-panel`。
4. Nuxt 前台最终只能走 Go API；现存 `/wp-json/**` 调用按模块逐步替换。
