...
本项目仍处在 WordPress/PHP 到 Go 的迁移过程中。根目录 WordPress 主题壳文件和 `style.css` 主题元数据已经删除；剩余 PHP 目录保留的意义是兼容旧线上能力和查询旧业务逻辑，不是新增功能入口。

硬性规则：

1. 不恢复根目录 WordPress 主题壳文件或 `style.css` 主题元数据，不在 `wp-plugin/**` 添加、修改、回调核心业务逻辑。
...
- 先屏蔽旧 route/menu。
- 再删除旧插件模块文件。
- 根目录主题壳和 `style.css` 已删除；剩余删除工作只针对仍需兼容的旧插件模块。
