...
export function useFitmentFrameLookup() {
  const { t } = useI18n()
  const { fetchFrameEntries } = useFitmentCatalogApi()
  const searchQuery = ref('')
  const year = ref<number | null>(null)
  const entries = ref<FitmentFrameEntry[]>([])
  ...
  const load = async (page = 1) => {
    ...
    try {
      const response = await fetchFrameEntries({
      ...
      entries.value = response.data?.frame_entries || []
