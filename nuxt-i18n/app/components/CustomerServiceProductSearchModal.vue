  ...
  <Teleport to="body">
    <Transition name="customer-service-product-search-modal">
      ...
      >
        ...
        <section
          ref="modalElement"
          class="customer-service-product-search-modal-shell tz-mobile-dialog-surface relative flex tz-mobile-safe-drawer-90-height w-full flex-col overflow-hidden border border-x-0 border-b-0 tz-border-subtle tz-surface-card tz-text-primary shadow-[0_24px_80px_rgba(15,23,42,0.16)] md:h-auto md:max-h-[min(42rem,calc(100dvh-1.5rem))] md:w-[min(56rem,calc(100vw-1.5rem))] md:border"
          role="dialog"
          aria-modal="true"
        ...
        >
          <button
            type="button"
            class="flex h-9 w-full items-center justify-center border-b tz-border-subtle tz-text-primary/75 transition-colors hover:tz-text-primary md:hidden"
          ...
          <header class="flex items-start justify-between gap-4 border-b tz-border-subtle px-4 py-4 md:px-6">
            <div class="min-w-0">
              <h2 :id="modalTitleId" class="text-base font-semibold tz-text-primary md:text-lg">
                {{ t('chatModal.productPicker.title') }}
              </h2>
              <p class="mt-1 text-sm leading-5 tz-text-muted">
            ...
            <button
              type="button"
              class="hidden h-9 w-9 shrink-0 items-center justify-center border tz-border-subtle tz-text-muted transition-colors hover:tz-border-strong/50 hover:tz-text-primary md:flex"
          ...
          <form
            class="flex gap-2 border-b tz-border-subtle px-4 py-3 md:px-6"
            @submit.prevent="handleCustomerServiceProductSearchSubmit"
          >
            <div class="relative min-w-0 flex-1">
              <Icon
                name="lucide:search"
                class="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 tz-text-primary/45"
              />
              <input
                ...
                class="h-11 w-full border tz-border-subtle tz-surface-subtle pl-10 pr-10 text-sm tz-text-primary outline-none transition-colors placeholder:tz-text-primary/35 focus:border-[#059669]/70"
                autocomplete="off"
              />
              <button
                v-if="searchQuery"
                type="button"
                class="absolute right-2 top-1/2 flex h-7 w-7 -translate-y-1/2 items-center justify-center tz-text-primary/45 transition-colors hover:tz-text-primary"
          ...
          <div class="min-h-0 flex-1 overflow-y-auto p-4 md:p-6">
            <div
              v-if="isSearching"
              class="flex min-h-48 flex-col items-center justify-center gap-3 text-sm tz-text-muted"
            >
              <Icon name="lucide:loader-circle" class="h-6 w-6 animate-spin text-[#059669]" />
              <span>{{ t('chatModal.productPicker.searching') }}</span>
            </div>

            <div
              v-else-if="searchError"
              class="flex min-h-48 items-center justify-center text-center text-sm text-red-700"
            >
              {{ searchError }}
            </div>
            ...
            <div
              ...
              class="flex min-h-48 items-center justify-center text-center text-sm tz-text-primary/50"
            >
            ...
            <div v-else class="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
              <article
                ...
                :key="product.id"
                class="flex min-h-56 flex-col overflow-hidden border tz-border-subtle tz-surface-card transition-colors hover:border-[#059669]/55"
              >
                <div class="aspect-[4/3] w-full tz-surface-subtle">
                  <StorefrontImage
                ...
                <div class="flex flex-1 flex-col p-3">
                  <h3 class="line-clamp-2 text-sm font-semibold leading-5 tz-text-primary">
                    {{ product.title }}
                  </h3>
                  <p v-if="product.priceLabel" class="mt-1 text-sm font-semibold text-[#059669]">
                    {{ product.priceLabel }}
                  </p>
                  <p v-if="product.sku" class="mt-1 truncate text-xs tz-text-primary/45">
                    {{ product.sku }}
                  </p>

                  <button
                    type="button"
                    class="mt-auto flex h-9 items-center justify-center gap-2 border tz-border-strong/30 tz-surface-subtle px-3 text-xs font-semibold tz-text-primary transition-colors hover:border-[#059669]/70 hover:bg-[#059669]/10"
