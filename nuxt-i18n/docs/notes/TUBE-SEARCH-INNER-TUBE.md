...
The old WordPress / site settings / WP REST `tube_specs` plan is no longer the active architecture. our storefront search now follows the Nuxt + Go backend product-specification-template path.
...
- Inner tube products are selected by stable product specification template slug, currently `inner-tube`.
...
6. `/shop` builds the product query with `product_specification_template=<slug>` when the selected category is a real product specification template.
7. Go backend filters products in `internal/repository/product_query_repository.go` by joining `product_specification_templates.slug`.

If `inner-tube` is not present in `GET /api/v1/products/specification-templates`, the flow safely degrades to keyword search instead of inventing a fake category.
...
- `app/pages/shop/index.vue`
  - owns `selectedCategory`, product query building, and the final `product_specification_template=<slug>` request parameter.
- `app/composables/useShopCategories.ts`
  - reads `GET /api/v1/products/specification-templates`; DEV fallback is development-only and must not become product-specification-template truth in production.
...
- `internal/api/v1/product/handler.go`
  - public product specification template and product listing handlers.
- `internal/repository/product_attribute_repository.go`
  - returns enabled product specification templates ordered by `sort_order ASC, id ASC`.
- `internal/repository/product_query_repository.go`
  - filters public products by `product_specification_templates.slug` when `product_specification_template` is provided.
- `internal/domain/product/model.go`
  - `Product`, `ProductSpecificationTemplate`, spec definitions, and variant structures.
...
- The product request uses the single backend query parameter `product_specification_template=<slug>`.
- No old WordPress `tube_specs` table, WP REST endpoint, or site settings plugin page is part of the current path.

## Still not fully closed

1. Product specification template data verification
   - DEV / staging must contain an enabled product specification template with slug `inner-tube` before the CTA can narrow results by category.
...
2. Precise tube filters
   ...
   - If future tube products need valve family, valve length, execution, ETRTO range, or size filters, define them through the Go product specification template/spec template model first.
...
- Keep `inner-tube` as a stable slug unless backend product specification template data is deliberately changed.
- If the slug changes, update product specification template data, `InnerTubeGuide.vue`, `ProductSearchPanel.vue`, this document, and the `/shop` category verification together.
