...
- Customer-service lifecycle and inbox archive contract: `../../docs/design/customer-service-conversation-lifecycle-and-inbox-architecture.md`
- Chat product configuration confirmation flow: `notes/CHAT_CONFIG_CONFIRM_DESIGN.md`
- Storefront light-theme color tokens: `../../docs/design/emerald-light-theme-palette.md`
...
- Warranty: current Nuxt + Go backend boundary is documented in `notes/PRODUCT-WARRANTY-SYSTEM.md`. Admin backend ownership, public upload validation, admin UI consumption, order-number lookup, order-item claim linkage, and service records are closed; remaining work is richer claim processing UX and e2e tests.
- Shop categories: current implementation is documented in `notes/SHOP_CATEGORY_LAYOUT.zh-CN.md`. DEV fallback policy and empty-state copy are closed; remaining work is verification against real product-specification-template data and future backend-owned i18n handling for dynamic category names.
