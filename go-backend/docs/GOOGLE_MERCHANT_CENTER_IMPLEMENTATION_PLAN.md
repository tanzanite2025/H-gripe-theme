...
Google, while product clicks continue to land on the Tanzanite storefront.
...
The channel workspace is now separated from the core Tanzanite catalog:
...
Tanzanite remains the source of truth for all catalog, price, inventory,
shipping, and storefront URL data.

Google Merchant Center is the distribution and diagnostics system. Google must
never become the primary source for Tanzanite product descriptions, pricing, or
stock.

One Tanzanite product or variant must map deterministically to one external
...
- Reconcile Tanzanite offers against Google on a scheduled basis.
...
- MCP access cannot disclose credentials or change Tanzanite catalog data.
