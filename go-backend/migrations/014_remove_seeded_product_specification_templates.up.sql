-- Product specification templates are managed by administrators and must not be pre-populated.
...
WHERE spec_definition_id IN (
    ...
    WHERE product_specification_template_id IN (
        SELECT id FROM product_specification_templates WHERE slug IN ('carbon_rim', 'frame')
    )
);
