...
func TestProductSupplierCostCatalogRepositoryReturnsMinimalVariantProjection(t *testing.T) {
	db := newProductSupplierCostCatalogTestDB(t)
	repo := NewProductSupplierCostCatalogRepository(db)

	_, total, err := repo.ListOptions(ProductSupplierCostCatalogFilter{
		Page:     1,
		PageSize: 20,
		Search:   "picker",
	})
	...
	options, _, err := repo.ListOptions(ProductSupplierCostCatalogFilter{
		Page:     1,
		PageSize: 20,
		Search:   "picker",
	})
...
func TestProductSupplierCostCatalogRepositoryExactSKUCanReopenInactiveHistory(t *testing.T) {
	db := newProductSupplierCostCatalogTestDB(t)
	repo := NewProductSupplierCostCatalogRepository(db)

	options, total, err := repo.ListOptions(ProductSupplierCostCatalogFilter{
		Page:     1,
		PageSize: 20,
...
func TestProductSupplierCostCatalogRepositoryDefaultListExcludesUnavailableVariants(t *testing.T) {
	db := newProductSupplierCostCatalogTestDB(t)
	repo := NewProductSupplierCostCatalogRepository(db)
...
func newProductSupplierCostCatalogTestDB(t *testing.T) *gorm.DB {
	t.Helper()
