...
func (s *UGCShowcaseService) CleanupExpiredPendingImages(
	...
	for _, item := range candidates {
		...
		if item.Status == ugcshowcasedomain.StatusPending {
			updated, err := s.repo.UpdatePendingStatus(item.ID, ugcshowcasedomain.StatusRejected, showcaseExpiredPendingReason)
			if err != nil {
				return result, err
			}
			if !updated {
