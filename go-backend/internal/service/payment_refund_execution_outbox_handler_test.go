...
func TestRefundExecutionGatewayConfigUsesDomainManagedEncryptedConfig(t *testing.T) {
	masterKey := "refund-worker-payment-config-test-master-key"
	...
	adminSettings := newRefundWorkerPaymentSettingsService(t)
	...
	config, err := refundExecutionGatewayConfig(adminSettings, pgateway.GatewayPayPal)
...
func newRefundWorkerPaymentSettingsService(t *testing.T) *AdminSettingsService {
	t.Helper()
	...
	return NewAdminSettingsService(settingService)
}
