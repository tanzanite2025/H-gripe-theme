...
func (b *dependencyServicesBuilder) build() error {
	...
	lighthouseRunnerService := service.NewLighthouseRunnerService(
		...
		service.LighthouseRunnerConfig{
			...
			RunnerTimeout:               time.Duration(b.cfg.SiteQuality.RunnerTimeoutSeconds) * time.Second,
			StorefrontBaseURL:           storefrontBaseURL,
			StorefrontTargetURL:         siteQualityTargetOrigin,
			ThrottlingMethod:            b.cfg.SiteQuality.ThrottlingMethod,
			LighthouseRunCount:          b.cfg.SiteQuality.LighthouseRunCount,
			RenderWaitSelector:          b.cfg.SiteQuality.RenderWaitSelector,
			RenderWaitTimeout:           time.Duration(b.cfg.SiteQuality.RenderWaitTimeoutMilliseconds) * time.Millisecond,
			HeadingSettleTimeout:        time.Duration(b.cfg.SiteQuality.HeadingSettleMilliseconds) * time.Millisecond,
			StructuredDataSettleTimeout: time.Duration(b.cfg.SiteQuality.StructuredDataSettleMilliseconds) * time.Millisecond,
			InteractionProbes:           b.cfg.SiteQuality.InteractionProbes,
			InteractionMaxResponse:      time.Duration(b.cfg.SiteQuality.InteractionMaxResponseMillis) * time.Millisecond,
			SoftNavigationSelectors:     b.cfg.SiteQuality.SoftNavigationSelectors,
			SoftNavigationMaxLinks:      b.cfg.SiteQuality.SoftNavigationMaxLinks,
			SoftNavigationMaxDuration:   time.Duration(b.cfg.SiteQuality.SoftNavigationMaxDurationMillis) * time.Millisecond,
			SoftNavigationMaxHeapGrowth: b.cfg.SiteQuality.SoftNavigationMaxHeapGrowthMB,
			JSBudgetBytes:               b.cfg.SiteQuality.JSBudgetBytes,
			ImageBudgetBytes:            b.cfg.SiteQuality.ImageBudgetBytes,
			LinkCheckEnabled:            b.cfg.SiteQuality.LinkCheckEnabled,
			LinkCheckMaxLinks:           b.cfg.SiteQuality.LinkCheckMaxLinks,
			LinkCheckTimeout:            time.Duration(b.cfg.SiteQuality.LinkCheckTimeoutMillis) * time.Millisecond,
			LinkCheckExternal:           b.cfg.SiteQuality.LinkCheckExternal,
			LinkCheckMaxRedirects:       b.cfg.SiteQuality.LinkCheckMaxRedirects,
		},
	)
