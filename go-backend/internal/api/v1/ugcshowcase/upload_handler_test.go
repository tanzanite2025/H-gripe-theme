...
func TestShowcaseUploadFailsClosedWithoutUploadProtection(t *testing.T) {
	...
	handler := NewUGCShowcaseHandler(nil)
	router := gin.New()
...
func TestShowcaseUploadValidRequestEvaluatesQuotaAfterValidation(t *testing.T) {
	...
	uploadService := &showcaseUploadServiceSpy{
		events: &events,
		item: &ugcshowcasedomain.UGCShowcase{
			ID:     99,
...
type showcaseUploadProtectionSpy struct {
	evaluateCalls int
	failureCalls  int
	lastInput     service.UGCShowcaseUploadProtectionInput
	events        *[]string
}

func (s *showcaseUploadProtectionSpy) Evaluate(_ context.Context, input service.UGCShowcaseUploadProtectionInput) (service.UGCShowcaseUploadProtectionDecision, error) {
	s.evaluateCalls++
	s.lastInput = input
	if s.events != nil {
		*s.events = append(*s.events, "evaluate")
	}
	return service.UGCShowcaseUploadProtectionDecision{
		Allowed: true,
		Action:  service.UGCShowcaseUploadProtectionDecisionAllow,
	}, nil
}

func (s *showcaseUploadProtectionSpy) RecordFailure(_ context.Context, _ service.UGCShowcaseUploadProtectionIdentity) error {
	s.failureCalls++
	return nil
}
...
func (s *showcaseUploadServiceSpy) UploadPhotos(_ context.Context, _ uint, _ uint, _ []*multipart.FileHeader, _ map[string]string) (*ugcshowcasedomain.UGCShowcase, error) {
	s.uploadCalls++
	if s.events != nil {
...
func serveShowcaseUploadTestRequest(handler *ugcShowcaseHandler, request *http.Request) *httptest.ResponseRecorder {
	router := gin.New()
	router.POST("/showcase/upload", func(c *gin.Context) {
		c.Set("user_id", uint(42))
		handler.Upload(c)
