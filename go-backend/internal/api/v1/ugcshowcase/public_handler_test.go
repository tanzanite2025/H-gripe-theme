...
func TestShowcasePublicListReturnsControlledImageURLs(t *testing.T) {
	...
	require.NoError(t, db.AutoMigrate(&ugcshowcasedomain.UGCShowcase{}, &ugcshowcasedomain.UGCShowcaseComment{}))
	require.NoError(t, db.Create(&ugcshowcasedomain.UGCShowcase{
		UserID: 1,
		Kind:   ugcshowcasedomain.KindUser,
		Status: ugcshowcasedomain.StatusApproved,
		Images: datatypes.JSON(`["showcase/approved/2026/08/13/approved.webp"]`),
	}).Error)

	handler := NewUGCShowcaseHandler(service.NewUGCShowcaseService(repository.NewUGCShowcaseRepository(db), nil))
	router := gin.New()
...
func TestShowcasePublicImageHandlerServesApprovedImage(t *testing.T) {
	...
	handler := NewUGCShowcaseHandler(service.NewUGCShowcaseService(
		repository.NewUGCShowcaseRepository(db),
