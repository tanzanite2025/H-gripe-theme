...
func (h *ExchangeRateHandler) ConvertDisplayPrices(c *gin.Context) {
	...
	amountMoney, err := domainmoney.ParseMajor(req.AmountDecimal, baseCurrency)
	if err != nil || amountMoney.AmountMinor() <= 0 {
		apierror.RespondBadRequest(c, "amount_decimal must be a positive amount")
		return
	}
	...
	for _, quoteCurrency := range quoteCurrencies {
		convertedMoney, err := h.exchangeRateService.ConvertMoneyStrict(amountMoney, quoteCurrency)
		if err != nil {
			apierror.RespondError(c, http.StatusUnprocessableEntity, "exchange_rate_unavailable", err.Error())
			return
		}
		_, formatErr := convertedMoney.FormatMajor()
		if formatErr != nil {
			apierror.RespondError(c, http.StatusUnprocessableEntity, "exchange_rate_unavailable", formatErr.Error())
			return
		}
		prices = append(prices, exchangeRateDisplayPrice{
			AmountDecimal: func() string { value, _ := convertedMoney.FormatMajor(); return value }(),
	...
	response.Success(c, gin.H{
		"amount_decimal":   req.AmountDecimal,
