...
func registerOperationsRoutes(
	...
	opsGroup.Use(middleware.RequirePermission(auth.PermOpsView))
	{
		...
		opsGroup.GET("/outbox/failures", outboxReconciliationHandler.ListFailures)
		opsGroup.GET("/outbox/failures/:id", outboxReconciliationHandler.GetFailure)
		opsGroup.POST("/outbox/failures/:id/retry", middleware.RequirePermission(auth.PermSystemManage), outboxReconciliationHandler.RetryFailure)
		opsGroup.POST("/outbox/failures/:id/ignore", middleware.RequirePermission(auth.PermSystemManage), outboxReconciliationHandler.IgnoreFailure)
