...
const (
	YearModeSingle  YearMode = "single"
	YearModeRange   YearMode = "range"
	YearModeAll     YearMode = "all"
	YearModeUnknown YearMode = "unknown"
)

type FrameFitmentEntry struct {
...
func (FrameFitmentEntry) TableName() string {
	return "fitment_frame_entries"
}

func (entry *FrameFitmentEntry) BeforeCreate(tx *gorm.DB) error {
	return entry.Normalize()
}

func (entry *FrameFitmentEntry) BeforeSave(tx *gorm.DB) error {
	return entry.Normalize()
}

func (entry *FrameFitmentEntry) Normalize() error {
	if entry == nil {
		return fmt.Errorf("frame fitment entry is nil")
	}
...
func (entry *FrameFitmentEntry) Validate() error {
	if entry == nil {
		return fmt.Errorf("frame fitment entry is nil")
	}
