<template>
  <section class="border bg-card">
    <div class="flex flex-wrap items-center justify-between gap-3 border-b px-4 py-3">
      <div>
        <h2 class="text-sm font-black">链接文字检查</h2>
        <p class="mt-1 text-xs text-muted-foreground">按 Lighthouse 官方 link-text 规则检查非描述性链接文字</p>
      </div>
      <div class="flex items-center gap-2">
        ...
        <Button size="icon" variant="outline" title="刷新链接文字检查" :disabled="loading" @click="$emit('refresh')">
      ...
      >
        <div class="min-w-0">
          <div class="flex flex-wrap items-center gap-2">
            ...
            <p class="truncate text-sm font-black">{{ linkRuleLabel(finding.rule_id || finding.audit_id) }}</p>
          ...
          <p class="mt-1 truncate font-mono text-[10px] text-muted-foreground" :title="finding.provider_audit_id || finding.audit_id">
            provider={{ finding.provider_audit_id || finding.audit_id }}
          </p>
...
import { ChevronLeft, ChevronRight, Eye, Link2, RefreshCw } from '@lucide/vue'
import AdminStatusBadge, { type AdminStatusTone } from '@/components/admin/AdminStatusBadge.vue'
import { Button } from '@/components/ui/button'
import { SITE_QUALITY_RULE_ID_DESCRIPTIVE_LINK_TEXT } from '@/api/preflight'
import type {
  SiteQualityFinding,
...
const linkRuleLabel = (ruleID: string): string => (
  ruleID === SITE_QUALITY_RULE_ID_DESCRIPTIVE_LINK_TEXT ? '链接缺乏描述性文字' : ruleID
)
