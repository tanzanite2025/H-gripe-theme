<template>
  <section class="space-y-3">
    <div class="border border-dashed border-border/80 bg-card p-4">
      <div class="flex flex-wrap items-start justify-between gap-3">
        <div>
          <p class="text-[10px] font-black uppercase tracking-[0.18em] text-muted-foreground">Effective Risk Strategy Policy</p>
          <h2 class="mt-1 text-lg font-black tracking-tight">当前生效风控策略</h2>
 ...
 <dd class="mt-1 text-sm font-semibold">{{ gateway.health?.allowed === false ? '当前拦截': '允许调用'}}</dd>
    ...
    <div class="grid grid-cols-1 gap-3 xl:grid-cols-3">
      ...
      <section class="border border-dashed border-border/80 bg-card p-4">
        <h3 class="text-xs font-black uppercase tracking-widest">风控策略预检</h3>
        <p class="mt-1 text-[11px] leading-5 text-muted-foreground">
 ...
 <dd class="font-semibold">{{ riskStrategyPrecheck.enabled ? '已开启': '未开启'}}</dd>
          </div>
          <div class="flex items-start justify-between gap-3 border-b border-dashed border-border/70 pb-2">
            <dt class="text-muted-foreground">失败计数窗口</dt>
            <dd class="font-mono font-semibold">{{ seconds(riskStrategyPrecheck.failure_window_seconds) }}</dd>
          </div>
          <div class="flex items-start justify-between gap-3 border-b border-dashed border-border/70 pb-2">
            <dt class="text-muted-foreground">重复失败阈值</dt>
            <dd class="font-mono font-semibold">{{ number(riskStrategyPrecheck.failure_threshold) }} 次</dd>
          </div>
          <div class="flex items-start justify-between gap-3 border-b border-dashed border-border/70 pb-2">
            <dt class="text-muted-foreground">高风险分数</dt>
            <dd class="font-mono font-semibold">≥ {{ number(riskStrategyPrecheck.high_risk_score) }}</dd>
          </div>
          <div class="flex items-start justify-between gap-3">
            <dt class="text-muted-foreground">命中后延迟</dt>
            <dd class="font-mono font-semibold">{{ seconds(riskStrategyPrecheck.delay_seconds) }}</dd>
          </div>
        </dl>
      </section>
 ...
 <dd class="font-semibold">{{ antiAbuse.turnstile_required ? (antiAbuse.turnstile_configured ? '已启用且已配置': '启用但缺密钥') : '未强制'}}</dd>
          ...
          <div class="flex items-start justify-between gap-3 border-b border-dashed border-border/70 pb-2">
            ...
            <dd class="font-mono font-semibold">{{ number(riskStrategyPrecheck.failure_threshold) }} 次 / ≥ {{ number(riskStrategyPrecheck.high_risk_score) }} 分</dd>
          </div>
          <div class="flex items-start justify-between gap-3">
            <dt class="text-muted-foreground">命中后处理</dt>
            <dd class="font-semibold">延迟 {{ seconds(riskStrategyPrecheck.delay_seconds) }}，必要时站点挑战</dd>
          </div>
        </dl>
      </section>
...
const riskStrategyPrecheck = computed(() => configuration.value.payment_risk || {})
...
const gatewayStatusLabel = (gateway: { runtime?: RiskStrategyGatewayRuntimeStatus | null; health?: { allowed?: boolean; circuit_open?: boolean } | null }): string => {
...
const gatewayStatusClass = (gateway: { runtime?: RiskStrategyGatewayRuntimeStatus | null; health?: { allowed?: boolean; circuit_open?: boolean } | null }): string => {
