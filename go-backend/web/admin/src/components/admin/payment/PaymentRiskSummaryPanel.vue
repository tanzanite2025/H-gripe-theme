  ...
  <section class="space-y-3">
    <div class="border border-dashed border-border/80 bg-card p-4">
      <div class="flex flex-wrap items-start justify-between gap-3">
        <div>
          <p class="text-[10px] font-black uppercase tracking-[0.18em] text-muted-foreground">Payment Risk Observability</p>
          <h2 class="mt-1 text-lg font-black tracking-tight">支付风控实时总览</h2>
      ...
      <div class="mt-3 border-t border-dashed border-border/70 pt-3 text-[11px] leading-5 text-muted-foreground">
        ...
        成功支付取已完成交易；Stripe 争议合并拒付表与标准化拒付事件；EFW 取支付风险事件；退款取已完成退款；
    ...
    <section class="grid grid-cols-1 gap-3 lg:grid-cols-2">
      ...
      >
        ...
        <div v-else class="mt-5 border border-dashed border-border/80 p-4 text-sm text-muted-foreground">
          {{ enabled ? '等待定时任务或新的支付风险事件生成首次计算。' : '支付风险监控当前未启用。' }}
        </div>
      </article>
    </section>
...
import type {
  PaymentRiskMonitoringPolicy,
  PaymentRiskProviderReport,
  PaymentRiskReports,
  PaymentRiskSnapshot,
...
const props = withDefaults(defineProps<{
  ...
  policy?: PaymentRiskMonitoringPolicy | null
  enabled?: boolean
  loading?: boolean
}>(), {
  reports: () => ({}),
