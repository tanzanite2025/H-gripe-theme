<template>
 ...
 <div class="mt-1 text-muted-foreground">{{ log.ip_address || '-'}}</div>
  ...
  <RiskStrategyProtectionConfirmDialog
...
import RiskStrategyProtectionConfirmDialog from '@/components/admin/payment/RiskStrategyProtectionConfirmDialog.vue'
...
const riskConfiguration = ref<RiskStrategyConfiguration | null>(null)
...
const refresh = async (): Promise<void> => {
  loading.value = true
  try {
    const payload = await riskStrategyApi.listProtectionControls(true)
    ...
    try {
      const summary = await riskStrategyApi.getSummary()
      ...
      console.error('Failed to fetch risk strategy policy summary:', error)
...
const selectControl = async (control: PaymentProtectionControl): Promise<void> => {
  ...
  const payload = await riskStrategyApi.listProtectionControlAudit(control.id)
...
const submitCreateControl = async (): Promise<void> => {
  ...
  saving.value = true
  try {
    await riskStrategyApi.createProtectionControl(pendingCreatePayload.value)
...
const revokeControl = async (): Promise<void> => {
  ...
  saving.value = true
  try {
    await riskStrategyApi.revokeProtectionControl(selectedControl.value.id)
