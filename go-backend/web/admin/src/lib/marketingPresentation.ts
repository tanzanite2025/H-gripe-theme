...
export const couponValue = (coupon: CouponLike): string => coupon.type === 'percentage' ? `${formatMoney(coupon.value)}%` : `¥${formatMoney(coupon.value)}`
...
export const giftCardStatusName = (status?: string | null): string => ({ active: '活跃', used: '已使用', expired: '已过期', cancelled: '已取消' })[status || ''] || status || '-'
export const giftCardStatusTone = (status?: string | null): StatusTone => ({ active: 'green', used: 'blue', expired: 'amber', cancelled: 'coral' } as Record<string, StatusTone>)[status || ''] || 'gray'
export const transactionTypeName = (type?: string | null): string => ({ issue: '发行', use: '消费', refund: '退款' })[type || ''] || type || '-'
export const loyaltyTypeName = (type?: string | null): string => ({ earn: '获得', spend: '消费', expire: '过期', adjust: '调整', refund: '退回' })[type || ''] || type || '-'

export const giftCardStatusOptions = (card?: GiftCardLike | null): StatusOption[] => {
