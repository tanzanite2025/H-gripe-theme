...
export interface ProductSupplierCostRecord {
  id: number
  product_code: string
  product_name: string
...
export type ProductSupplierCostRecordUpdatePayload = ProductSupplierCostRecordDetailsPayload

export interface ProductSupplierCostProductOption {
  product_name: string
...
export interface ProductSupplierCostPagination {
  page: number
  page_size: number
  total: number
  total_pages: number
...
export interface ProductSupplierCostRecordListPayload {
  records: ProductSupplierCostRecord[]
  pagination: ProductSupplierCostPagination
}

const readRecord = (value: unknown, endpoint: string): ProductSupplierCostRecord => {
  const record = requireApiObject(value, endpoint, 'record') as ProductSupplierCostRecord
...
const readProductOption = (value: unknown, endpoint: string): ProductSupplierCostProductOption => {
  const option = requireApiObject(value, endpoint, 'product option') as ProductSupplierCostProductOption
...
export const productSupplierCostApi = {
  ...
  async listByCodes(codes: string[]): Promise<ProductSupplierCostRecord[]> {
    ...
    return requireApiArrayField<ProductSupplierCostRecord>(body, 'records', endpoint)
      .map((record) => readRecord(record, endpoint))
  },

  async listProductOptions(params: Record<string, unknown> = {}): Promise<{
    options: ProductSupplierCostProductOption[]
    pagination: ProductSupplierCostPagination
  }> {
  ...
  async get(id: number | string): Promise<ProductSupplierCostRecord> {
...
export default productSupplierCostApi
