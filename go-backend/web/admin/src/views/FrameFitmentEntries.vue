<template>
  <div class="space-y-4">
    <AdminPageHeader
      :title="t('fitmentCatalog.frame.title')"
      :description="t('fitmentCatalog.frame.description')"
    >
      <template #actions>
        <Button v-if="canCreate" @click="openCreate">
          <Plus class="size-4" />
          {{ t('fitmentCatalog.frame.create') }}
        </Button>
      </template>
    </AdminPageHeader>
      ...
      <form class="grid gap-3 md:grid-cols-[minmax(260px,1.5fr)_minmax(150px,0.55fr)_auto]" @submit.prevent="applyFilters">
        <label class="block space-y-1">
          ...
          <div class="relative">
            ...
            <Input v-model="filters.search" class="h-9 pl-9" :placeholder="t('fitmentCatalog.frame.searchPlaceholder')" />
          </div>
        </label>
    ...
    <AdminTablePanel :loading="loading">
      <Table class="min-w-[1120px]">
        <TableHeader>
          <TableRow>
            <TableHead class="w-56">{{ t('fitmentCatalog.columns.brandModel') }}</TableHead>
            ...
            <TableHead class="w-28">{{ t('fitmentCatalog.columns.rearHubSpecs') }}</TableHead>
        ...
        <TableBody>
          <TableEmpty v-if="entries.length === 0" :colspan="8">
            <div class="flex flex-col items-center text-muted-foreground">
              <Bike class="mb-2 size-7 opacity-55" />
              <span class="text-xs">{{ filters.search ? t('fitmentCatalog.frame.emptySearch') : t('fitmentCatalog.frame.empty') }}</span>
            </div>
          </TableEmpty>
          ...
          <TableRow v-for="entry in entries" :key="entry.id">
            ...
            <TableCell>
              <div class="min-w-0">
                <p class="truncate text-xs font-bold">{{ entry.series_name || t('fitmentCatalog.frame.unfilledSeries') }}</p>
            ...
            <TableCell class="font-mono text-xs text-muted-foreground">
              {{ entry.market_code || t('fitmentCatalog.frame.globalMarket') }}
            </TableCell>
            <TableCell class="font-mono text-xs font-bold">
            ...
            <TableCell class="text-right">
              <DropdownMenu>
                <DropdownMenuTrigger as-child>
                  <Button variant="ghost" size="icon" :aria-label="t('fitmentCatalog.frame.manageAria', { name: frameDisplayName(entry) })">
    ...
    <Dialog v-model:open="dialogOpen">
      <DialogContent size="lg">
        <DialogHeader>
          <DialogTitle>{{ form.id ? t('fitmentCatalog.frame.editTitle') : t('fitmentCatalog.frame.createTitle') }}</DialogTitle>
          <DialogDescription>
            {{ t('fitmentCatalog.frame.dialogDescription') }}
          </DialogDescription>
        </DialogHeader>

        <form class="space-y-5" @submit.prevent="save">
          <div class="grid gap-3 md:grid-cols-2">
            <AdminFormField :label="t('fitmentCatalog.fields.brand')" required :error="formErrors.brand_name">
              <Input v-model="form.brand_name" :disabled="saving" :placeholder="t('fitmentCatalog.placeholders.brand')" />
            </AdminFormField>
            <AdminFormField :label="t('fitmentCatalog.fields.model')" required :error="formErrors.model_name">
              <Input v-model="form.model_name" :disabled="saving" :placeholder="t('fitmentCatalog.placeholders.model')" />
            </AdminFormField>
              ...
              >
                ...
                <span class="min-w-0">
                  ...
                  <span v-if="!specification.is_enabled" class="mt-1 block text-[10px] font-bold text-amber-700 dark:text-amber-300">
                    {{ t('fitmentCatalog.frame.disabledHubNote') }}
                  </span>
                </span>
              </label>
            </div>
          ...
          <div class="flex items-center justify-between gap-4 rounded-lg border bg-muted/20 px-3 py-3">
            <div>
              <p class="text-xs font-black">{{ t('fitmentCatalog.frame.enabledTitle') }}</p>
              <p class="mt-1 text-[10px] text-muted-foreground">{{ t('fitmentCatalog.frame.enabledDescription') }}</p>
            </div>
            <Switch v-model:checked="form.is_enabled" :disabled="saving || !form.id" :aria-label="t('fitmentCatalog.frame.enabledTitle')" />
          </div>
...
import { type FitmentYearMode, type FrameFitmentEntry, type FrameFitmentEntryPayload, type HubSpecification } from '@/api/fitmentCatalog'
import { frameFitmentEntriesApi } from '@/api/fitmentCatalog/frameEntries'
...
const entries = ref<FrameFitmentEntry[]>([])
...
const form = reactive<FrameForm>(emptyForm())
...
const assignForm = (entry?: FrameFitmentEntry) => {
...
const loadEntries = async () => {
  loading.value = true
  try {
    const result = await frameFitmentEntriesApi.listFrameEntries(listParams())
    entries.value = result.entries
    Object.assign(pagination, result.pagination)
  } catch (error) {
    toast.error(error instanceof Error ? error.message : t('fitmentCatalog.frame.toast.loadFailed'))
  } finally {
    loading.value = false
  }
}
...
const loadHubOptions = async () => {
  ...
  try {
    ...
    toast.error(error instanceof Error ? error.message : t('fitmentCatalog.frame.toast.hubLoadFailed'))
  } finally {
...
const openEdit = async (entry: FrameFitmentEntry) => {
  try {
    const fullEntry = await frameFitmentEntriesApi.getFrameEntry(entry.id)
...
const toPayload = (): FrameFitmentEntryPayload => ({
...
const validateForm = (payload: FrameFitmentEntryPayload): boolean => {
  clearFormErrors()
  if (!payload.brand_name) formErrors.brand_name = t('fitmentCatalog.frame.validation.brand')
  if (!payload.model_name) formErrors.model_name = t('fitmentCatalog.frame.validation.model')
  if (!payload.year_mode) formErrors.year_mode = t('fitmentCatalog.frame.validation.yearMode')
  if ((payload.year_mode === 'single' || payload.year_mode === 'range') && payload.year_from === null) {
    formErrors.year_from = t('fitmentCatalog.frame.validation.yearFrom')
  }
  if (payload.year_mode === 'range' && payload.year_to === null) {
    formErrors.year_to = t('fitmentCatalog.frame.validation.yearTo')
  }
  if (payload.year_mode === 'range' && payload.year_from !== null && payload.year_to !== null && payload.year_from > payload.year_to) {
    formErrors.year_to = t('fitmentCatalog.frame.validation.yearRange')
  }
  if (payload.is_enabled && payload.hub_specification_ids.length === 0) {
    formErrors.hub_specification_ids = t('fitmentCatalog.frame.validation.hubSpecs')
  }
...
const frameDisplayName = (entry: FrameFitmentEntry): string => `${entry.brand_name} / ${entry.model_name}`
...
const save = async () => {
  ...
  try {
    if (form.id) {
      await frameFitmentEntriesApi.updateFrameEntry(form.id, payload)
      toast.success(t('fitmentCatalog.frame.toast.saved'))
    } else {
      await frameFitmentEntriesApi.createFrameEntry(payload)
      toast.success(t('fitmentCatalog.frame.toast.created'))
    }
    dialogOpen.value = false
    await loadEntries()
  } catch (error) {
    toast.error(error instanceof Error ? error.message : t('fitmentCatalog.frame.toast.saveFailed'))
  } finally {
    saving.value = false
  }
}

const toggleStatus = async (entry: FrameFitmentEntry) => {
  try {
    await frameFitmentEntriesApi.updateFrameEntryStatus(entry.id, !entry.is_enabled)
    toast.success(entry.is_enabled ? t('fitmentCatalog.frame.toast.disabled') : t('fitmentCatalog.frame.toast.enabled'))
    await loadEntries()
  } catch (error) {
    toast.error(error instanceof Error ? error.message : t('fitmentCatalog.frame.toast.statusFailed'))
  }
}

const removeEntry = async (entry: FrameFitmentEntry) => {
  if (!window.confirm(t('fitmentCatalog.frame.confirmDelete', { name: frameDisplayName(entry) }))) return
  try {
    await frameFitmentEntriesApi.removeFrameEntry(entry.id)
    toast.success(t('fitmentCatalog.frame.toast.deleted'))
    if (entries.value.length === 1 && pagination.page > 1) pagination.page -= 1
    await loadEntries()
  } catch (error) {
    toast.error(error instanceof Error ? error.message : t('fitmentCatalog.frame.toast.deleteFailed'))
  }
}

const formatYear = (entry: FrameFitmentEntry): string => {
