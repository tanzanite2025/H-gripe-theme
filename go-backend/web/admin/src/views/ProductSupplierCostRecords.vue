<template>
  <div class="space-y-4">
    <AdminPageHeader
      title="商品成本"
      description="独立维护 SKU 的单位成本价、供应商资料、到货周期、起订量和附加成本，不写入商品目录。"
    >
      <template #actions>
      ...
      <Table class="min-w-[1120px]">
        <TableHeader>
          <TableRow>
            ...
            <TableHead class="w-40">单位成本价</TableHead>
    ...
    <Dialog v-model:open="dialogOpen">
      <DialogContent size="lg">
        <DialogHeader>
          ...
          <DialogDescription>
            新增时从真实商品变体中选择 SKU；名称和变体标题由商品目录只读回填，成本币种与成本资料仍由本附加域独立维护。
          </DialogDescription>
        </DialogHeader>
        ...
        <form class="space-y-5" @submit.prevent="save">
          ...
          <section class="space-y-3">
            <div>
              ...
              <p class="mt-1 text-[10px] text-muted-foreground">单位成本价与供应商资料只在这个独立附加域中维护。</p>
            </div>
          ...
          <section class="space-y-3">
            <div>
              ...
              <p class="mt-1 text-[10px] text-muted-foreground">只记录产品成本资料，不生成供应商订单或库存数量。</p>
            </div>
            <div class="grid gap-3 md:grid-cols-2">
...
import productSupplierCostApi, {
  type ProductSupplierCostProductOption,
  type ProductSupplierCostRecord,
  type ProductSupplierCostRecordCreatePayload,
  type ProductSupplierCostRecordDetailsPayload,
} from '@/api/productSupplierCost'
...
import {
  LEGACY_PRODUCT_SUPPLIER_COST_CREATE_PERMISSION_CODE,
...
const canCreate = computed(() => authStore.hasPermission(LEGACY_PRODUCT_SUPPLIER_COST_CREATE_PERMISSION_CODE))
const canEdit = computed(() => authStore.hasPermission(LEGACY_PRODUCT_SUPPLIER_COST_EDIT_PERMISSION_CODE))
const canDelete = computed(() => authStore.hasPermission(LEGACY_PRODUCT_SUPPLIER_COST_DELETE_PERMISSION_CODE))
const currencies = ['USD', 'CNY', 'EUR', 'GBP', 'JPY']

const records = ref<ProductSupplierCostRecord[]>([])
...
const form = reactive<ProductSupplierCostForm>(emptyForm())
const productSearch = ref('')
const productOptions = ref<ProductSupplierCostProductOption[]>([])
const selectedProductOption = ref<ProductSupplierCostProductOption | null>(null)
...
function emptyForm(): ProductSupplierCostForm {
  return {
    sku: '',
    product_name: '',
    unit_cost: '',
...
const assignForm = (record?: ProductSupplierCostRecord): void => {
...
const loadProductOptions = async (reset = true): Promise<void> => {
  ...
  try {
    const payload = await productSupplierCostApi.listProductOptions({
    ...
    if (requestVersion !== productOptionRequestVersion.value) return
    console.error('Failed to fetch supplier-cost product options:', error)
...
const selectProductOption = (option: ProductSupplierCostProductOption): void => {
  if (productOptionSearchTimer) {
    clearTimeout(productOptionSearchTimer)
    productOptionSearchTimer = undefined
  }
...
const loadHistoricalProductOption = async (record: ProductSupplierCostRecord): Promise<void> => {
  const requestVersion = historicalProductRequestVersion.value + 1
  historicalProductRequestVersion.value = requestVersion
  try {
    const payload = await productSupplierCostApi.listProductOptions({ sku: record.product_code, page: 1, page_size: 1 })
    ...
    if (requestVersion !== historicalProductRequestVersion.value) return
    console.error('Failed to resolve historical supplier-cost product option:', error)
...
const fetchRecords = async (): Promise<void> => {
  loading.value = true
  try {
    const payload = await productSupplierCostApi.list({
      page: pagination.page,
    ...
    console.error('Failed to fetch supplier-cost records:', error)
...
const openEdit = (record: ProductSupplierCostRecord): void => {
  historicalProductRequestVersion.value += 1
  productPickerOpen.value = false
  assignForm(record)
  productSearch.value = ''
...
const save = async (): Promise<void> => {
  ...
  const unitCost = rawUnitCost === ''
    ? null
    : Number(rawUnitCost)
  if ((!form.id && !selectedProductOption.value) || !form.sku.trim() || unitCost == null || Number.isNaN(unitCost)) {
    toast.error(form.id ? '请确认商品快照并填写单位成本价' : '请选择真实商品变体 SKU 并填写单位成本价')
    return
  }
  saving.value = true
  try {
    ...
    if (form.id) {
      ...
      const payload: ProductSupplierCostRecordCreatePayload = {
        sku: form.sku.trim(),
        ...detailsPayload,
      }
      await productSupplierCostApi.create(payload)
    }
    dialogOpen.value = false
    toast.success('商品成本已保存')
    await fetchRecords()
  } catch (error) {
    console.error('Failed to save supplier-cost record:', error)
...
const removeRecord = async (record: ProductSupplierCostRecord): Promise<void> => {
  if (!window.confirm(`确定删除“${record.product_name}”的商品成本？`)) return
  try {
    await productSupplierCostApi.remove(record.id)
    toast.success('商品成本已删除')
    if (records.value.length === 1 && pagination.page > 1) pagination.page -= 1
    await fetchRecords()
  } catch (error) {
    console.error('Failed to delete supplier-cost record:', error)
