...
import productSupplierCostProfitabilityApi, {
  type ProfitabilityBulkUpsertResult,
  type ProfitabilityItemPayload,
  type ProfitabilitySupplierCostDetailsPayload,
} from '@/api/productSupplierCostProfitability'
...
export const useProductSupplierCostProfitDraft = () => {
  const draftsByKey = reactive<Record<string, ProductSupplierCostProfitDraft>>({})
  ...
  const rowsForVariants = (
    ...
    productName: string,
    currency: string,
  ): ProductSupplierCostProfitDraft[] => syncDrafts(variants, productName, currency)
    .map((key) => draftsByKey[key])

  const mergeLoadedRecords = (
    supplierCostRecords: Awaited<ReturnType<typeof productSupplierCostProfitabilityApi.listSupplierCostRecordsByCodes>>,
    profitabilityRecords: Awaited<ReturnType<typeof productSupplierCostProfitabilityApi.listProfitabilityByCodes>>,
  ) => {
    const profitabilityByCode = new Map(profitabilityRecords.map((record) => [record.product_code, record]))
    supplierCostRecords.forEach((record) => {
  ...
  const loadForProduct = async (product: ProductSnapshot | null): Promise<void> => {
    ...
    loading.value = true
    try {
      const [supplierCostRecords, profitabilityRecords] = await Promise.all([
        productSupplierCostProfitabilityApi.listSupplierCostRecordsByCodes(codes),
        productSupplierCostProfitabilityApi.listProfitabilityByCodes(codes),
      ])
      mergeLoadedRecords(supplierCostRecords, profitabilityRecords)
    } catch (error) {
  ...
  const hasSupplierCostInput = (draft: ProductSupplierCostProfitDraft): boolean => draft.unitCostKnown

  const toSupplierCostDetailsPayload = (draft: ProductSupplierCostProfitDraft): ProfitabilitySupplierCostDetailsPayload => ({
  ...
  const saveItems = async (
    ...
    saveError.value = ''
    try {
      const result = await productSupplierCostProfitabilityApi.bulkUpsert(items, requestId)
      pending.value = false
      pendingItems.value = null
      ...
      pendingItems.value = items.map((item) => ({
        ...item,
        supplier_cost_details: item.supplier_cost_details ? { ...item.supplier_cost_details } : undefined,
      }))
...
export default useProductSupplierCostProfitDraft
