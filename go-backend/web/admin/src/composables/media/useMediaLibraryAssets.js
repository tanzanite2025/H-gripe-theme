...
export const useMediaLibraryAssets = () => {
  ...
  const assets = ref([])
  const filters = reactive({ search: '', mediaType: 'all', status: 'all' })
  ...
  const saveAsset = async (asset, editor) => {
  ...
  const copyURL = async (asset, accessURL) => {
  ...
  const downloadCopyrightEvidence = async (asset) => {
    try {
