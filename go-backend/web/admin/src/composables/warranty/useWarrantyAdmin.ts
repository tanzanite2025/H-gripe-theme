...
import type { WarrantyStats } from '@/api/registrations'
import type {
  WarrantyClaim,
  WarrantyFilters,
  WarrantyID,
...
type WarrantyTab = 'registrations' | 'claims' | 'expiring' | 'boundary'
...
export function useWarrantyAdmin() {
  ...
  const stats = ref<WarrantyStats>({})
  const registrations = ref<WarrantyRegistration[]>([])
  const claims = ref<WarrantyClaim[]>([])
  const expiring = ref<WarrantyRegistration[]>([])
  ...
  const registrationFilters = reactive<WarrantyFilters>({
    status: 'all'
  })
  ...
  const registrationPagination = reactive<WarrantyPagination>({
    page: 1,
    pageSize: 20,
    total: 0
  })
  ...
  const fetchExpiring = async (): Promise<void> => {
  ...
  const updateRegistrationStatus = async (registration: WarrantyRegistration, status: string): Promise<void> => {
  ...
  const updateRegistrationPage = (page: number): void => {
    registrationPagination.page = page
  }

  const updateRegistrationPageSize = (pageSize: number): void => {
