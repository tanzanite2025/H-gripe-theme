...
- 页面功能：
  ...
  - 自动预填部分内容：
    ...
    - 默认正文可以包含摘要 + 链接 + 退订说明。
  - 显示本次预计发送对象数（当前订阅且未退订的邮箱数量）。
  - 点击「发送」按钮后：
    - 通过内部封装好的发送函数，对所有符合条件的订阅者发送邮件；
    - 可记录一次「广播日志」，包括时间、类型（博客/商品）、目标 ID、已发送数量等（日志可以作为后续扩展，不是第一版必需）。
...
1. 在一个独立插件或 `tanzanite-setting` 里：
   - 写好订阅表的安装代码（activation hook）。
2. 实现 REST API：`subscribe / confirm / unsubscribe`，用 Postman 或浏览器先测通。
3. 在 Nuxt 里接入订阅表单 UI，连上 `POST /subscribe`，验证前后端联通。
4. 加上博客发布通知 `publish_post` 的钩子，先用测试邮箱观察是否正常发出。
5. 在 tanzanite-setting 里确定商品上架的关键点，加上 `do_action('tanz_new_product_published', $product_id)`，并实现监听和邮件发送。
6. 视需要逐步优化邮件模板、频率控制和后台管理界面。
